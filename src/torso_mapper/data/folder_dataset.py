...
class FolderCTScanIterableDataset(IterableDataset):
    ...
    def _get_nii_files(self) -> List[str]:
        ...
        return [os.path.join(self.folder_path, f) for f in os.listdir(self.folder_path) if f.endswith('.nii') or f.endswith('.nii.gz')]
    ...
    def __iter__(self):
        ...
        for path in worker_file_paths:
            ...
            for start in range(-start_offset, depth, self.stride):
                ...
                block[block_start:block_end, :, :] = volume_slice
